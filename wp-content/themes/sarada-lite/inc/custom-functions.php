...
function sarada_lite_setup() {
    ...
    */    
    ...
    add_image_size( 'sarada-blog-sec', 675, 405, true );
    add_image_size( 'sarada-slider-two', 1545, 779, true );
    add_image_size( 'sarada-slider-three', 670, 585, true );
    add_image_size( 'sarada-single-two', 825, 495, true );
    add_image_size( 'sarada-single-three', 1170, 700, true );
...
function sarada_lite_scripts() {
    ...
    wp_enqueue_style( 'sarada-google-fonts', sarada_lite_fonts_url(), array(), null );
    wp_enqueue_style( 'sarada-lite', get_stylesheet_uri(), array(), SARADA_LITE_THEME_VERSION );

    if( sarada_lite_is_woocommerce_activated() )
    wp_enqueue_style( 'sarada-woocommerce', get_template_directory_uri(). '/css' . $build . '/woocommerce' . $suffix . '.css', array(), SARADA_LITE_THEME_VERSION );
    
    if( sarada_lite_is_elementor_activated() ){
        wp_enqueue_style( 'sarada-elementor', get_template_directory_uri(). '/css' . $build . '/elementor' . $suffix . '.css', array(), SARADA_LITE_THEME_VERSION );
    }
    
    wp_enqueue_style( 'sarada-gutenberg', get_template_directory_uri(). '/css' . $build . '/gutenberg' . $suffix . '.css', array(), SARADA_LITE_THEME_VERSION );
...
function sarada_lite_admin_scripts( $hook ){
    wp_enqueue_style( 'sarada-admin', get_template_directory_uri() . '/inc/css/admin.css', '', SARADA_LITE_THEME_VERSION );
}
endif; 
