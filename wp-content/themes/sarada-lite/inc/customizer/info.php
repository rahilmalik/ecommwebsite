...
function sarada_lite_customizer_theme_info( $wp_customize ) {
    ...
    $theme_info = '<p>';
	$theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'sarada-lite' ),  '<a href="' . esc_url( 'https://blossomthemesdemo.com/sarada-lite/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p><p>';
    $theme_info .= sprintf( __( 'Documentation Link: %1$sClick here.%2$s', 'sarada-lite' ),  '<a href="' . esc_url( 'https://docs.blossomthemes.com/docs/sarada-lite/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p>';
	...
	$manager->add_section(
		new Sarada_Lite_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => esc_url( 'https://blossomthemes.com/downloads/sarada/' ),
			)
		)
	);
}
