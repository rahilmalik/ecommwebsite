...
if ( ! class_exists( 'Shopay_Settings' ) ) :

	class Shopay_Settings {

		/**
		 * Constructor.
		 */
		public function __construct() {
			add_action( 'admin_menu', array( $this, 'shopay_admin_menu' ) );
			add_action( 'wp_loaded', array( __CLASS__, 'shopay_hide_notices' ) );
			add_action( 'wp_loaded', array( $this, 'shopay_admin_notice' ) );
		 ...
		 * Add admin menu.
		 */
		public function shopay_admin_menu() {
			$theme = wp_get_theme( get_template() );

			$page = add_theme_page( $theme->display( 'Name' ).' '.esc_html__( 'Settings', 'shopay' ), $theme->display( 'Name' ).' '.' '.esc_html__( 'Settings', 'shopay' ), 'activate_plugins', 'shopay-settings', array( $this, 'get_started_screen' ) );
		}
		...
		public function about_theme_styles( $hook ) {
			...
			wp_enqueue_style( 'mt-theme-review-notice', get_template_directory_uri() . '/inc/theme-settings/assets/css/theme-review-notice.css', array(), esc_attr( $shopay_theme_version ) );

			if ( 'appearance_page_shopay-settings' != $hook && 'themes.php' != $hook ) {
				return;
			}

			wp_enqueue_style( 'mt-theme-settings-style', get_template_directory_uri() . '/inc/theme-settings/assets/css/settings.css', array(), $shopay_theme_version );
		}
		...
		public function about_theme_scripts( $hook ) {
			...
			$theme_notice_option = get_option( 'shopay_admin_notice_welcome' );
			if ( $theme_notice_option ) {
				wp_enqueue_script( 'mt-theme-review-notice', get_template_directory_uri() . '/inc/theme-settings/assets/js/theme-review-notice.js', array( 'jquery' ), esc_attr( $shopay_theme_version ) );
				...
				wp_localize_script( 'mt-theme-review-notice', 'mtaboutObject', array(
					...
					'_wpnonce'	=> wp_create_nonce( 'shopay_admin_plugin_install_nonce' ),
					'action'	=> esc_html( $action )
				));
			}

			if ( 'appearance_page_shopay-settings' != $hook ) {
				return;
			}

			$activated_plugins = apply_filters( 'shopay_active_plugins', get_option('active_plugins') );
			...
			wp_enqueue_script( 'mt-theme-settings-script', get_template_directory_uri() . '/inc/theme-settings/assets/js/settings.js', array( 'jquery' ), esc_attr( $shopay_theme_version ) );
		 ...
		 * Add admin notice.
		 */
		public function shopay_admin_notice() {

			if ( isset( $_GET['activated'] ) ) {
				update_option( 'shopay_admin_notice_welcome', true );
			}
		...
		public static function shopay_hide_notices() {
			if ( isset( $_GET['shopay-hide-notice'] ) && isset( $_GET['_shopay_notice_nonce'] ) ) {
				...
				update_option( 'shopay_admin_notice_' . $hide_notice, false );
			}
		}
	...
	?>
			<div id="mt-theme-message" class="updated notice shopay-message">
				<a class="shopay-message-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'shopay-hide-notice', 'welcome' ) ), 'shopay_hide_notices_nonce', '_shopay_notice_nonce' ) ); ?>">
					<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'shopay' ); ?>
				</a>
				<h2 class="welcome-title"><?php printf( esc_html__( 'Welcome to %s', 'shopay' ), $theme_name ); ?></h2>
				<p>
					<?php printf( esc_html__( 'Welcome! Thank you for choosing %1$s ! To fully take advantage of the best our theme can offer please make sure you visit our %2$s theme settings page %3$s.', 'shopay' ), '<strong>'. esc_html( $theme_name ).'</strong>', '<a href="' . esc_url( admin_url( 'themes.php?page=shopay-settings' ) ) . '">', '</a>' ); ?>
				</p>
				<p>
					<?php printf( esc_html__( 'Clicking get started will process to installation of %1$s Mystery Themes Demo Importer %2$s Plugin in your dashboard. After success it will redirect to the theme settings page.', 'shopay' ), '<strong>', '</strong>' ); ?>
				</p>
				<div class="submit">
					<button class="mt-get-started button button-primary button-hero" data-done="<?php esc_html_e( 'Done!', 'shopay' ); ?>" data-process="<?php esc_html_e( 'Processing', 'shopay' ); ?>" data-redirect="<?php echo esc_url( wp_nonce_url( add_query_arg( 'shopay-hide-notice', 'welcome', admin_url( 'themes.php' ).'?page=shopay-settings&tab=demos' ) , 'shopay_hide_notices_nonce', '_shopay_notice_nonce' ) ); ?>">
						<?php printf( esc_html__( 'Get started with %1$s', 'shopay' ), esc_html( $theme_name ) ); ?>
					</button>
				</div>
	...
	?>
			<div class="shopay-theme-info mt-theme-info mt-clearfix">
			...
			<div class="mt-upgrader-pro">
				<div class="mt-upgrade-title-wrap">
					<h3 class="mt-upgrader-title"><?php esc_html_e( 'Upgrade to Premium Version', 'shopay' ); ?></h3>
					<div class="mt-upgrader-text"><?php esc_html_e( 'Upgrade to pro version for additional features and better supports.', 'shopay' ); ?></div>
				</div>
				<div class="mt-upgrader-btn"> <a href="<?php echo esc_url( 'https://mysterythemes.com/wp-themes/shopay-pro/' ); ?>" target="_blank" class="button button-primary"><?php esc_html_e( 'Unlock Features With Pro', 'shopay' ); ?></a> </div>
			</div><!-- .mt-upgrader-pro -->

			<div class="mt-nav-tab-content-wrapper">
				<div class="nav-tab-wrapper">

					<a class="nav-tab <?php if ( empty( $_GET['tab'] ) && $_GET['page'] == 'shopay-settings' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'shopay-settings' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-admin-appearance"></span> <?php esc_html_e( 'Get Started', 'shopay' ); ?>
					</a>

					<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'demos' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'shopay-settings', 'tab' => 'demos' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-download"></span> <?php esc_html_e( 'Demos', 'shopay' ); ?>
					</a>
					
					<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'free_vs_pro' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'shopay-settings', 'tab' => 'free_vs_pro' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-dashboard"></span> <?php esc_html_e( 'Free Vs Pro', 'shopay' ); ?>
					</a>

					<a class="nav-tab <?php if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'changelog' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'shopay-settings', 'tab' => 'changelog' ), 'themes.php' ) ) ); ?>">
						<span class="dashicons dashicons-flag"></span> <?php esc_html_e( 'Changelog', 'shopay' ); ?>
					</a>
				</div><!-- .nav-tab-wrapper -->
	<?php
		}
				...
				<?php $this->intro(); ?>
					<div class="mt-nav-content-wrap">
						<div class="theme-features-wrap welcome-panel">
							<h4><?php esc_html_e( 'Here are some useful links for you to get started', 'shopay' ); ?></h4>
							<div class="under-the-hood two-col">
								<div class="col">
									<h3><?php esc_html_e( 'Next Steps', 'shopay' ); ?></h3>
									<ul>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[panel]=shopay_theme_options_panel' ); ?>" target="_blank" class="welcome-icon dashicons-networking"><?php esc_html_e( 'Theme Options', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[section]=shopay_section_site_style' ); ?>" target="_blank" class="welcome-icon dashicons-admin-page"><?php esc_html_e( 'Setup Site Style', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[panel]=shopay_header_group' ); ?>" target="_blank" class="welcome-icon dashicons-editor-kitchensink"><?php esc_html_e( 'Manage header section', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[section]=shopay_section_sidebar_layout' ); ?>" target="_blank" class="welcome-icon dashicons-text-page"><?php esc_html_e( 'Sidebar Layouts', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'customize.php' ).'?autofocus[section]=shopay_section_social_icons' ); ?>" target="_blank" class="welcome-icon dashicons-networking"><?php esc_html_e( 'Manage Social Icons', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?>" target="_blank" class="welcome-icon welcome-menus"><?php esc_html_e( 'Manage menus', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>" target="_blank" class="welcome-icon welcome-widgets"><?php esc_html_e( 'Manage widgets', 'shopay' ); ?></a>
										</li>
									</ul>
								</div>

								<div class="col">
									<h3><?php esc_html_e( 'More Actions', 'shopay' ); ?></h3>
									<ul>
										<li>
											<a href="<?php echo esc_url( $doc_url ); ?>" target="_blank" class="welcome-icon dashicons-media-text"><?php esc_html_e( 'Documentation', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( $pro_theme_url ); ?>" target="_blank" class="welcome-icon dashicons-migrate"><?php esc_html_e( 'Premium version', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( $support_url ); ?>" target="_blank" class="welcome-icon dashicons-businesswoman"><?php esc_html_e( 'Need theme support?', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( $review_url ); ?>" target="_blank" class="welcome-icon dashicons-thumbs-up"><?php esc_html_e( 'Review theme', 'shopay' ); ?></a>
										</li>
										<li>
											<a href="<?php echo esc_url( 'https://wpallresources.com/' ); ?>" target="_blank" class="welcome-icon dashicons-admin-users"><?php esc_html_e( 'WP Tutorials', 'shopay' ); ?></a>
										</li>
									</ul>
								</div>
							</div>
	...
	<?php
		...
		public function demos_screen() {
			...
			$demodata 			= get_transient( 'shopay_demo_packages' );
			
			if ( empty( $demodata ) || $demodata == false ) {
				$shopay_library = new Shopay_Demo_Library();
				$demodata = $shopay_library->retrieve_demo_by_activatetheme();
				if ( $demodata ) {
					set_transient( 'shopay_demo_packages', $demodata, WEEK_IN_SECONDS );
				}
			}
						...
						<div class="mt-theme-demos rendered">
							...
							<?php
								if ( isset( $demodata ) && empty( $demodata ) ) {
									esc_html_e( 'No demos are configured for this theme, please contact the theme author', 'shopay' );
									return;
								} else {
							?>
													...
													<div class="mt-each-demo<?php if  ( strpos( $activated_theme, 'pro' ) == false && strpos( $theme_slug, 'pro' ) !== false ) { echo ' mt-demo-pro'; } ?> theme mtdi_gl-item js-ocdi-gl-item" data-categories="ltrdemo" data-name="<?php echo esc_attr ( $theme_slug ); ?>" style="display: block;">
														...
														<div class="theme-id-container">
															...
															<div class="mtdi-theme-actions theme-actions">
																...
																?>
																		<a class="button disabled button-primary hide-if-no-js" href="javascript:void(0);" data-name="<?php echo esc_attr ( $theme_name ); ?>" data-slug="<?php echo esc_attr ( $theme_slug ); ?>" aria-label="<?php printf ( esc_html__( 'Imported %1$s', 'shopay' ), $theme_name ); ?>">
																			<?php esc_html_e( 'Imported', 'shopay' ); ?>
																		</a>
																<?php
																	} else {
																...
																?>
																			<a class="button button-primary mtdi-purchasenow" href="<?php echo esc_url( $purchaseurl ); ?>" target="_blank" data-name="<?php echo esc_attr ( $theme_name ); ?>" data-slug="<?php echo esc_attr ( $theme_slug ); ?>" aria-label="<?php printf ( esc_html__( 'Purchase Now', 'shopay' ), $theme_name ); ?>">
																				<?php esc_html_e( 'Buy Now', 'shopay' ); ?>
																			</a>
																<?php
																		} else {
																			if ( is_plugin_active( 'mysterythemes-demo-importer/mysterythemes-demo-importer.php' ) ) {
																				$button_tooltip = esc_html__( 'Click to import demo', 'shopay' );
																			} else {
																				$button_tooltip = esc_html__( 'Demo importer plugin is not installed or activated', 'shopay' );
																			}
																?>
																			<a title="<?php echo esc_attr( $button_tooltip ); ?>" class="button button-primary hide-if-no-js mtdi-demo-import" href="javascript:void(0);" data-name="<?php echo esc_attr ( $theme_name ); ?>" data-slug="<?php echo esc_attr ( $theme_slug ); ?>" aria-label="<?php printf ( esc_attr__( 'Import %1$s', 'shopay' ), $theme_name ); ?>">
																				<?php esc_html_e( 'Import', 'shopay' ); ?>
																			</a>
																<?php
																		}
																	}
																?>
																	<a class="button preview install-demo-preview" target="_blank" href="<?php echo esc_url ( $demourl ); ?>">
																		<?php esc_html_e( 'View Demo', 'shopay' ); ?>
																	</a>
	...
	<?php
		...
		?>
			<div class="wrap about-wrap">
				...
				<?php $this->intro(); ?>
					<div class="mt-nav-content-wrap">
						<h4><?php esc_html_e( 'Upgrade to PRO version for more exciting features.', 'shopay' ); ?></h4>
						<table>
						<thead>
							<tr>
						...
						<tbody>
							<tr>
								...
								<td><?php esc_html_e( '$59.99', 'shopay' ); ?></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<a href="<?php echo esc_url( apply_filters( 'shopay_pro_theme_url', 'https://mysterythemes.com/wp-themes/shopay-pro/' ) ); ?>" class="button button-primary" target="_blank"><?php esc_html_e( 'Buy Pro', 'shopay' ); ?></a>
								</td>
							</tr>
						</tbody>
					</table>
	...
	<?php
	    ...
	    public function shopay_admin_footer_text( $text ) {
	        ...
	        if ( 'appearance_page_shopay-settings' == $screen->id ) {

	        	$theme 		= wp_get_theme( get_template() );
				$theme_name = $theme->get( 'Name' );

	            $text = sprintf( __( 'If you like <strong>%1$s</strong> please leave us a %2$s rating. A huge thank you from <strong>Mystery Themes</strong> in advance &#128515!', 'shopay' ), esc_html( $theme_name ), '<a href="https://wordpress.org/support/theme/shopay/reviews/?filter=5#new-post" class="theme-rating" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>' );
		...
		?>
				<div id="mt-demo-import-plugin-popup">
					<div class="mt-popup-inner-wrap">
						<?php
							if ( is_plugin_active( 'mysterythemes-demo-importer/mysterythemes-demo-importer.php' ) ) {
								echo '<span class="mt-plugin-message">'.esc_html__( 'You can import available demos now!', 'shopay' ).'</span>';
							} else {
								if ( ! file_exists( $demo_importer_plugin ) ) {
						?>
									<span class="mt-plugin-message"><?php esc_html_e( 'Mystery Themes Demo Importer Plugin is not installed!', 'shopay' ); ?></span>
									<a href="javascript:void(0)" class="mt-install-demo-import-plugin" data-process="<?php esc_attr_e( 'Installing & Activating', 'shopay' ); ?>" data-done="<?php esc_attr_e( 'Installed & Activated', 'shopay' ); ?>">
										<?php esc_html_e( 'Install and Activate', 'shopay' ); ?>
									</a>
						<?php
								} else {
						?>
									<span class="mt-plugin-message"><?php esc_html_e( 'Mystery Themes Demo Importer Plugin is installed but not activated!', 'shopay' ); ?></span>
									<a href="javascript:void(0)" class="mt-activate-demo-import-plugin" data-process="<?php esc_attr_e( 'Activating', 'shopay' ); ?>" data-done="<?php esc_attr_e( 'Activated', 'shopay' ); ?>">
										<?php esc_html_e( 'Activate Now', 'shopay' ); ?>
									</a>
						<?php
								}
							}
		...
		public function activate_demo_importer_plugin() {
			if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'shopay_admin_plugin_install_nonce' ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				wp_send_json_success(
					array(
						'success' => true,
						'message' => __( 'Plugin Successfully Activated.', 'shopay' ),
					)
				);
			}
		}
		...
		function install_demo_importer_plugin() {
			...
			if ( ! current_user_can( 'install_plugins' ) ) {
				$status['message'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'shopay' );
			...
			if ( is_wp_error( $result ) ) {
				...
				$status['message'] 		= __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'shopay' );
			...
			$status['message'] = esc_html__( 'Plugin installed successfully', 'shopay' );
