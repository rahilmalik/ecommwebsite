...
class VI_WOO_ORDERS_TRACKING_ADMIN_ORDERS_EDIT_TRACKING {
	private $settings;
	protected $carriers;
	protected $shipping_carrier_default;
	protected $custom_carriers_list;
	protected $shipping_carriers_define_list;

	public function __construct() {
		$this->settings                      = new VI_WOO_ORDERS_TRACKING_DATA();
		$this->carriers                      = array();
		$this->shipping_carrier_default      = $this->settings->get_params( 'shipping_carrier_default' );
		$this->custom_carriers_list          = $this->settings->get_params( 'custom_carriers_list' );
		$this->shipping_carriers_define_list = json_encode( VI_WOO_ORDERS_TRACKING_DATA::shipping_carriers() );
		$this->enqueue_action();
	}
			...
			if ( is_a( $screen, 'WP_Screen' ) && $screen->id == 'shop_order' ) {
				...
				wp_localize_script( 'vi-wot-admin-edit-order-js',
					...
					array(
						...
						'shipping_carrier_default'           => $this->shipping_carrier_default,
						'shipping_carrier_default_url_check' => $this->settings->get_shipping_carrier_url( $this->shipping_carrier_default, 'custom-carrier' ),
						'custom_carriers_list'               => $this->custom_carriers_list,
						'shipping_carriers_define_list'      => $this->shipping_carriers_define_list,
