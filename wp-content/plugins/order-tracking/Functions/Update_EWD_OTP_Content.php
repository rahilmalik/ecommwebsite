	...
	if (isset($_GET['OTPAction'])) {
		switch ($_GET['OTPAction']) {
			...
			case "EWD_OTP_DeleteStatus":
	       		$ewd_otp_message = Delete_EWD_OTP_Status(stripslashes_deep(urldecode($_GET['Status'])));
				break;
			...
			case "EWD_OTP_DeleteOrder":
	       		$ewd_otp_message = Delete_EWD_OTP_Order( intval( $_GET['Order_ID'] ) );
				break;
			case "EWD_OTP_HideOrder":
	       		$ewd_otp_message = Hide_EWD_OTP_Order( intval( $_GET['Order_ID'] ) );
				break;
			case "EWD_OTP_Delete_Order_Status":
	       		$ewd_otp_message = Delete_EWD_OTP_Order_Status( intval( $_GET['Order_Status_ID'] ) );
				break;
			...
			case "EWD_OTP_DeleteSalesRep":
	       		$ewd_otp_message = Delete_EWD_OTP_Sales_Rep( intval( $_GET['Sales_Rep_ID'] ) );
				break;
			...
			case "EWD_OTP_DeleteCustomer":
	       		$ewd_otp_message = Delete_EWD_OTP_Customer( intval( $_GET['Customer_ID'] ) );
				break;
			case "EWD_OTP_MassDeleteCustomers":
			...
			case "EWD_OTP_DeleteCustomField":
				$upcp_message = Delete_EWD_OTP_Custom_Field( intval( $_GET['Field_ID'] ) );
				break;
			...
			case "EWD_OTP_DeleteAllOrders":
				...
				break;
			default:
				$ewd_otp_message = __("The form has not worked correctly. Please contact the plugin developer.", 'order-tracking');
				break;
		}
	}
}

function Update_EWD_OTP_Non_Admin_Content() {
	if (isset($_GET['OTPAction'])) {
		switch ($_GET['OTPAction']) {
			case "EWD_OTP_ExportToExcel":
				$ewd_otp_message = EWD_OTP_Export_To_Excel();
